<template>
  <div v-if="$store.state.showOpenedPages" class="flex flex-items-center page-tabs">
    <el-tabs v-model="$store.state.nowPage" type="card" @tab-click="click" @tab-remove="remove">
      <el-tab-pane v-for="item in $store.state.openedPages" :key="item.meta.menuIdStr" :closable="item.path !== '/'"
        :label="item.meta.menuName" :name="item.meta.menuIdStr">
      </el-tab-pane>
    </el-tabs>
