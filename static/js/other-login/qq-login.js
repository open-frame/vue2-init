        ...
        return {
            stringify: function () {
                ...
                try {
                    ...
                    $Console.error("JSON.parse => parse鏁版嵁鏍煎紡閿欒:" + str);
                }
                return ret;
            }
        }
...
(function (_qc) {
    ...
    var config = {
        ...
        getCrossSolution: function () {
            ...
            if (window.postMessage) {
                ...
                _qc.Console.error("鏈壘鍒板彲鐢ㄧ殑璺ㄥ煙閫氫俊鏂规");
                solution = "EMPProxy";
            }
            _qc.Console.info("纭畾璺ㄥ煙浠ｇ悊绛栫暐锛�" + solution);
            return solution;
        }
    };
    var sequence = 1000;
    ...
    Response[100013] = function (dt, dtTxt) {
        _qc.Login.signOut();
        _qc.Console.warn("api杩斿洖token澶辨晥");
    };
    ...
    Proxy.prototype.send = function (req) {
        ...
        while (this.isReady && (pendingRequest = this.requests.shift())) {
            ...
            QC.Console.log("seq no :" + pendingRequest.seq + "璇锋眰鍙戣捣" + "  ts -> " + (+new Date()));
    ...
    Proxy.dispatchReceive = function (seq, resText, status, fmt) {
        ...
        for (var i = 0; i < pendingRequests.length; i++) {
            if (pendingRequests[i].seq == seq) {
                QC.Console.log("seq no :" + seq + "鍝嶅簲鏀跺埌" + "  ts -> " + (+new Date()));
    ...
    Proxy.invoke = function () {
        ...
        return function (cmdStr) {
            ...
            if (!_proxy) {
                _qc.Console.info("Proxy鏈垵濮嬪寲锛宨nvoke鍏ユ爤");
                Proxy.generateProxy();
                return;
            }
            var _crtIvk;
    ...
    _qc.Object.extend(FLAProxy.prototype, {
        prefix: '_TC_QC_flaProxy_', init: function () {
            var _me = this, conId = function () {
                ...
                _qc.Console.info("璺ㄥ煙绐楀彛鏍囪瘑鍙� __qc_wId : " + ret);
                return ret;
            ...
            window[this.prefix + 'onFlashReady_' + this.timeStamp] = function () {
                _qc.Console.info("FLAProxy浠ｇ悊鍒涘缓鎴愬姛锛岃€楁椂" + (new Date() - _me.timeStamp));
                setTimeout(function () {
                    _me.isReady = true;
                    _me.send();
                    _me.invoke();
            ...
            proxyFun ? proxyFun(uri, paras, method, fmt, seq) : (!function () {
                throw new Error("flash proxy 鍒濆鍖栧け璐�")
            }());
    ...
    _qc.Object.extend(PMProxy.prototype, {
        init: function () {
            ...
            _frame_on_load.fire = function () {
                _qc.Console.info("PMProxy浠ｇ悊鍒涘缓鎴愬姛锛岃€楁椂" + (new Date() - _me.timeStamp));
                _me.isReady = true;
                _me.send();
                _me.invoke();
            };
    ...
    _qc.api = function () {
        ...
        var bindTokenPara = function (req) {
            ...
            if (appId <= 0) throw new Error("鎰忓鐨勮皟鐢ㄤ簡缁戝畾token鍒皉eq鐨勬柟娉� bindTokenPara");
            ...
            if (appId > 0) {
                setTimeout(function () {
                    ...
                    if (aToken.openid && aToken.accessToken) {
                        ...
                        _qc.Console.log("openid涓巃ccessToken涓㈠け锛岃皟鐢ㄨ姹傚叆鏍� : [" + api + "]锛屾爤澶у皬锛�" + _pendingPool.length);
                    }
                }, 10);
            } else {
                _pendingPool.push(req);
                _qc.Console.log((_proxy.isReady && appId < 0 ? "token鑾峰彇澶辫触锛岃璋冪敤鐢ㄦ埛鐧诲綍娴佺▼" : "api浠ｇ悊灏氭湭鍒濆鍖栨垚鍔�") + "锛岃皟鐢ㄨ姹傚叆鏍� : [" + api + "]锛屾爤澶у皬锛�" + _pendingPool.length);
            }
        ...
        fun._ready = function () {
            _qc.Console.info("init鎴愬姛锛屽紑濮嬭Е鍙慳pi璋冪敤鍫嗘爤");
            var _crtReq;
    ...
    var Login = function () {
                    ...
                    if (!opts.showModal) {
                        ...
                        btn.firstChild.onclick = function(){
                            ...
                            if (!iframe.src) {
                                ...
                                params += "response_type=token&" // 鑾峰彇鍥瀉ccess_token
                                params += "client_id=" + appid +"&" // 搴旂敤鐨刟ppid
                                params += "redirect_uri=" + opts.redirectURI // 鐧诲綍鎴愬姛鍚庤烦杞殑浠ｇ悊椤甸潰
            ...
            return access_token || function () {
                ...
                if ((__qc__k = $Cookie.get("__qc__k"))) {
                    ...
                    if (__qc__k.length && _keys.length != 2) {
                        throw new Error("QQConnect -> cookie : __qc__k 鏍煎紡闈炴硶");
                    }
                }
        ...
        var _getMe = function () {
            ...
            var getOpenId = function (_cb) {
                ...
                window.callback = function (_dt) {
                    ...
                    $Console.log(" getMe => openId & accessToken " + [openId, aToken, script ? '閫氳繃me鎺ュ彛' : '閫氳繃鏈湴']);
                    var cCb;
                ...
                if (dt_cache) {
                    ...
                    script.onerror = function () {
                        _getMeError("me鎺ュ彛杩斿洖鏍煎紡閿欒");
                    };
                    var _head = document.getElementsByTagName("head")[0];
                    _head && _head.appendChild(script);
                    _timer = setTimeout(function () {
                        _getMeError("me鎺ュ彛瓒呮椂");
                    }, 5000);
                }
            };
            getOpenId.cbPool = [];
        ...
        var retFun = function (opts, loginFun, logoutFun, __outCallFlag) {
            var args = arguments;
            var result = null;
            // 鍏佽鍦↙ogin鍒濆鍖栨椂浼犲叆access_token 2018-8-16 zhiyonglao
            ...
            !__outCallFlag && logoutFun !== null && _logoutFuns.push(function (__opts) {
                return function () {
                    ...
                    // 淇敼涓€涓嬬櫥鍑洪€昏緫 2018-8-16 zhiyonglao
                ...
                access_token ? _getMe(function (openId) {
                    if (openId) {
                        QC.api("get_user_info").success(function (req) {
                            ...
                            QC.Console.error("Login => getMe 鑾峰彇鏁版嵁澶辫触" + req);
                        });
                    } else {
                        _insertButton(opts);
                    }
        ...
        $Object.extend(retFun, {
            insertButton: _insertButton, getMe: _getMe, showPopup: _showPopup, signOut: function () {
                ...
                var dom = $(btnId),
                    _logoutTemplate = ['<span class="qc_item figure"><img src="{figureurl}" class="{size_key}"/></span>', '<span class="qc_item nickname" style="margin-left:6px;">{nickname}</span>', '<span class="qc_item logout"><a href="javascript:QC.Login.signOut();" style="margin-left:6px;">閫€鍑�</a></span>'].join("");
    ...
    _qc.init = function (cfg) {
        ...
        _qc.Login.getMe(function (oid, token, dt) {
            if (!~~dt.error && (dt.client_id <= 0 || dt.client_id % 1000000 == cfg.appId % 1000000)) {
                ...
                _qc.Console.error(dt.error_description || "appId/client_id 涓嶅尮閰�");
            }
        }, true);
    };
    ...
    (function () {
        ...
        if (aid) {
            _qc.Console.info("妫€娴嬪埌鑷姩鍒濆鍖栧弬鏁癨nappId:" + aid + "\nrUri:" + rUri);
            if (isNaN(aid)) {
                _qc.Console.error("鍙傛暟appid閿欒");
            } else if (rUri && rUri.indexOf("http") != 0) {
                _qc.Console.error("鍙傛暟rediectURI閿欒");
            } else {
